...
function normalizeEscuela(raw: any): Conjuro['Escuela'] {
...
function normalizeDisciplina(raw: any): Conjuro['Disciplina'] {
    if (!raw || typeof raw !== 'object')
...
export function normalizeConjuro(raw: any): Conjuro {
    return {
        Id: toNumber(raw?.Id ?? raw?.i),
        Nombre: toText(raw?.Nombre ?? raw?.n),
        ...
        Escuela: normalizeEscuela(raw?.Escuela ?? raw?.es),
        Disciplina: normalizeDisciplina(raw?.Disciplina ?? raw?.di),
